...
export async function getLatestPosts(count = 3) {
  const res = await fetch(
    `${WP_API}/posts?per_page=${count}&_fields=id,title,slug,excerpt,date,categories&_embed=wp:term`,
    { next: { revalidate: 3600 } }
  );
  if (!res.ok) return [];
  return res.json();
...
export async function getPostCount() {
  const res = await fetch(`${WP_API}/posts?per_page=1`, { next: { revalidate: 3600 } });
  if (!res.ok) return 93;
  return parseInt(res.headers.get('X-WP-Total') || '93');
}